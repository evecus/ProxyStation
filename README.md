...
<img src="frontend/public/proxystation-logo.png" width="120" alt="ProxyStation Logo">
...
```bash
curl -fsSL https://raw.githubusercontent.com/evecus/P-Panel/main/install.sh | sudo bash
```

The script will:
- Detect system architecture automatically (amd64/arm64)
- Download the latest stable release
- Install to `/etc/proxystation`
...
| Platform | File |
|:---|:---|
| Linux x64 | `ProxyStation-linux-amd64.tar.gz` |
| Linux ARM64 | `ProxyStation-linux-arm64.tar.gz` |

```bash
# Extract and run
tar -xzf ProxyStation-*.tar.gz
cd ProxyStation-*
./ProxyStation
```
...
To run ProxyStation from source or contribute to development:
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/star8618/ProxyStation.git
   cd ProxyStation
   ```
