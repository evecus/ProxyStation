...
func ConfigureFirefoxProxy() error {
	...
	for _, profile := range profiles {
		...
		//   4 = 自动检测
		//   5 = 使用系统代理
		proxyConfig := `// ProxyStation Auto Configuration - Firefox System Proxy
// 自动配置 Firefox 使用系统代理
...
`
		...
		// 移除旧的 ProxyStation 配置（如果有）
		content := string(existingContent)
		if idx := strings.Index(content, "// ProxyStation Auto Configuration"); idx != -1 {
			// 找到配置块的结束位置
...
func ClearFirefoxProxy() error {
	...
	for _, profile := range profiles {
		...
		if idx := strings.Index(newContent, "// ProxyStation Auto Configuration"); idx != -1 {
			// 找到下一个空行或文件结束
