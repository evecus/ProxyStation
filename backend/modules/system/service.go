...
func (s *Service) IsAutoStartEnabled() bool {
	...
	cmd := exec.Command("systemctl", "is-enabled", "proxystation")
	output, _ := cmd.Output()
...
Description=ProxyStation Proxy Gateway
...
`, s.binaryPath, filepath.Dir(s.binaryPath))
	...
	servicePath := "/etc/systemd/system/proxystation.service"
	...
	// 启用服务
	if err := exec.Command("systemctl", "enable", "proxystation").Run(); err != nil {
...
func (s *Service) disableAutoStart() error {
	exec.Command("systemctl", "disable", "proxystation").Run()
	exec.Command("systemctl", "stop", "proxystation").Run()
	os.Remove("/etc/systemd/system/proxystation.service")
...
func (s *Service) persistSysctl(keyValues ...string) error {
	configPath := "/etc/sysctl.d/99-proxystation.conf"
	...
	content.WriteString("# ProxyStation 系统优化配置\n")
