...
// CustomRuleEntry 用户自定义规则集条目（Mihomo 用）
type CustomRuleEntry struct {
	Name        string `json:"name"`        // 规则名称，如 my-rule
	URL         string `json:"url"`         // 下载地址
	Behavior    string `json:"behavior"`    // domain / ipcidr / classical
	Format      string `json:"format"`      // mrs / yaml / text
...
type RuleSetConfig struct {
	...
	CustomRules    []CustomRuleEntry `json:"customRules"`   // 用户自定义规则集
}
...
// GetCustomRules 获取用户自定义规则列表（附带本地文件状态）
func (s *Service) GetCustomRules() []RuleFile {
	s.mu.RLock()
	entries := append([]CustomRuleEntry{}, s.config.CustomRules...)
	s.mu.RUnlock()
	...
	for _, e := range entries {
		f := RuleFile{
			...
			Path:        filepath.Join(s.rulesetDir, "custom-"+e.Name+"."+e.Format),
		}
		f = s.updateFileStatus(f)
		files = append(files, f)
	}
	return files
}

// GetCustomRuleEntries 获取原始自定义规则条目（供配置生成使用）
func (s *Service) GetCustomRuleEntries() []CustomRuleEntry {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return append([]CustomRuleEntry{}, s.config.CustomRules...)
}

// AddCustomRule 添加自定义规则
func (s *Service) AddCustomRule(entry CustomRuleEntry) error {
	if entry.Name == "" || entry.URL == "" {
		return fmt.Errorf("name 和 url 不能为空")
	}
	if entry.Behavior == "" {
	...
	// 不允许重名
	for _, e := range s.config.CustomRules {
		if e.Name == entry.Name {
			return fmt.Errorf("规则 %q 已存在", entry.Name)
		}
	}

	s.config.CustomRules = append(s.config.CustomRules, entry)
	return s.saveConfig()
}

// DeleteCustomRule 删除自定义规则
func (s *Service) DeleteCustomRule(name string) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	rules := s.config.CustomRules
	for i, e := range rules {
		if e.Name == name {
			s.config.CustomRules = append(rules[:i], rules[i+1:]...)
			// 同时删除本地缓存文件（如果有）
			for _, ext := range []string{"mrs", "srs", "yaml", "text"} {
				os.Remove(filepath.Join(s.rulesetDir, "custom-"+name+"."+ext))
			}
			return s.saveConfig()
		}
	}
