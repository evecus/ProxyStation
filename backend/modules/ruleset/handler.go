...
func (h *Handler) RegisterRoutes(r *gin.RouterGroup) {
	group := r.Group("/ruleset")
	{
		...
		// 自定义规则集
		group.GET("/custom", h.GetCustomRules)
		group.POST("/custom", h.AddCustomRule)
		group.DELETE("/custom/:name", h.DeleteCustomRule)
	}
}
...
// GetCustomRules 获取自定义规则列表
func (h *Handler) GetCustomRules(c *gin.Context) {
	files := h.service.GetCustomRules()
	entries := h.service.GetCustomRuleEntries()
	c.JSON(http.StatusOK, gin.H{
		"code":    0,
		"message": "success",
		"data": gin.H{
...
func (h *Handler) AddCustomRule(c *gin.Context) {
	var entry CustomRuleEntry
	...
	if err := h.service.AddCustomRule(entry); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"code":    -1,
			"message": err.Error(),
		})
	...
	c.JSON(http.StatusOK, gin.H{
		"code":    0,
		"message": "已添加自定义规则: " + entry.Name,
	})
}

// DeleteCustomRule 删除自定义规则
func (h *Handler) DeleteCustomRule(c *gin.Context) {
	name := c.Param("name")
	if name == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"code":    -1,
	...
	if err := h.service.DeleteCustomRule(name); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"code":    -1,
			"message": err.Error(),
		})
