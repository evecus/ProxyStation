...
// singBoxCustomRulesProvider 获取用户自定义规则集（由 server 注入）
var singBoxCustomRulesProvider func() []CustomRuleEntry
...
// SetSingBoxCustomRulesProvider 设置自定义规则集提供者（由 server 注入）
func SetSingBoxCustomRulesProvider(fn func() []CustomRuleEntry) {
	singBoxCustomRulesProvider = fn
}
...
func handleGetRuleSets(c *gin.Context) {
	...
	// 合并用户自定义规则集
	if singBoxCustomRulesProvider != nil {
		for _, cr := range singBoxCustomRulesProvider() {
			format := cr.Format
			if format == "" {
				format = "srs"
			}
			localPath := filepath.Join(singBoxRulesetDir, "custom-"+cr.Name+"."+format)
			entry := RuleSetFileInfo{
				Tag:    "custom-" + cr.Name,
				Type:   "local",
				Format: "binary",
				URL:    cr.URL,
			...
			checkFileStatus(&entry.Path, &entry.Size, &entry.UpdatedAt, &entry.Status, &entry.Exists)
			defaultRuleSets = append(defaultRuleSets, entry)
		}
	}
