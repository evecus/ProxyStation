...
type ConfigGenerator struct {
	dataDir             string
	customRulesProvider func() []CustomRuleEntry // 获取自定义规则集的回调
}

// CustomRuleEntry 自定义规则条目（与 ruleset 包对应，避免循环依赖用本地定义）
...
// SetCustomRulesProvider 设置自定义规则获取回调
func (g *ConfigGenerator) SetCustomRulesProvider(fn func() []CustomRuleEntry) {
	g.customRulesProvider = fn
}
...
func (g *ConfigGenerator) generateRuleProviders() map[string]RuleProvider {
	...
	// 合并用户自定义规则集
	if g.customRulesProvider != nil {
		rulesetDir := filepath.Join(g.dataDir, "ruleset")
		for _, cr := range g.customRulesProvider() {
			format := cr.Format
			if format == "" {
				format = "mrs"
			}
			...
			localPath := filepath.Join(rulesetDir, "custom-"+cr.Name+"."+format)
			if _, err := os.Stat(localPath); err == nil {
				providers[cr.Name] = RuleProvider{
					Type:     "file",
					Behavior: behavior,
					Path:     localPath,
					Format:   format,
