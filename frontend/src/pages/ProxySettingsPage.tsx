...
import { 
  Settings, Shield, Zap, Globe, RefreshCw, 
  RotateCcw, ChevronDown, ChevronUp,
  Loader2, Network, Lock, Plus, Trash2, Radio, Shuffle, Laptop, Router
} from 'lucide-react'
import { proxySettingsApi, ProxySettings, AuthUser } from '@/api/proxySettings'
...
export default function ProxySettingsPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setIsRunning(statusData?.running ?? false)
    } catch (error) {
  ...
  const handleModeOrScopeChange = async (
    ...
    // mode=off 时强制 scope 为 local（UI 已禁用选择，但确保后端一致）
    const effectiveScope: ProxyScope = newMode === 'off' ? 'local' : newScope

    setTransparentMode(newMode)
    setProxyScope(effectiveScope)

    try {
      await proxyApi.setTransparentMode(newMode, effectiveScope)

      if (!settings) return
      const newSettings = { ...settings }

      // 根据代理作用域同步 allowLan
      newSettings.allowLan = effectiveScope === 'router'
        ...
        <div>
          ...
          {transparentMode !== 'off' && (
            <div className={cn(
              ...
              {isRunning ? (
                transparentMode === 'tproxy'
                  ? '✓ TProxy 模式运行中：nftables TPROXY 规则已激活，支持 TCP+UDP'
                  : '✓ Redir 模式运行中：nftables REDIRECT 规则已激活，仅 TCP'
              ) : (
                transparentMode === 'tproxy'
                  ? '○ TProxy 模式已配置，启动核心后自动添加 nftables TPROXY 规则'
                  : '○ Redir 模式已配置，启动核心后自动添加 nftables REDIRECT 规则'
              )}
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {transparentMode !== 'off' && proxyScope === 'router' && (
            <div className={cn(
              ...
              {isRunning
                ? '✓ 路由器模式运行中：allow-lan 已开启，Prerouting 链代理局域网设备，Output 链代理本机'
                : '○ 路由器模式已配置，启动核心后自动开启 allow-lan 和 Prerouting 链规则'}
            </div>
          )}
        </div>
      </div>
